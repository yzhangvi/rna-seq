...
请先重新部署最新版本。这个版本的上传解析不再依赖 Python 旧版 `cgi` 模块，适合新版 Python 和 Docker 公网环境。

如果仍然失败，看网页底部状态栏：

- `Failed to fetch` / `无法连接后端 API`：通常是只部署了静态网页，或者前端网页和 Python/R 后端不是同一个服务。请确认部署的是 Docker Web Service，而不是 Static Site。
- `HTTP 413`：文件太大，减少一次上传的文件数量，或在服务器环境变量中调高 `MAX_UPLOAD_MB`。
- `HTTP 502/503`：云平台内存不足、服务重启，或同步请求等待太久。最新版本已把 `运行分析` 改成后台任务轮询；如果仍然 502，优先升级到 4 GB RAM 以上实例并查看 Render Logs。
- `服务器没有返回可读取的数据`：通常是云平台返回了错误页面，请查看平台日志。

默认上传限制是 `MAX_UPLOAD_MB=200`。

可以打开以下地址检查后端是否在线：

```text
https://你的公网网址/api/health
```
